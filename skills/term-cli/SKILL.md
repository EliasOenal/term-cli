...
**Memory compaction?** If you lose context of what happened, **recall from terminal scrollback** with `term-cli capture --lines 200` (or more).
...
# ... do other work ...
term-cli wait --session dev --timeout 120 && term-cli capture -s dev
...
```bash
term-cli capture --session NAME --lines 200 --raw
```
...
```bash
term-cli wait --session NAME --timeout 30                     # default: 10s
```

**Use `wait-idle` for TUIs** (vim, htop, less) that don't have a detectable prompt — waits for screen to settle:

```bash
term-cli wait-idle --session NAME --seconds 2 --timeout 30    # defaults: 2s idle, 10s timeout
```

**Use `wait-for` sparingly** — waits for pattern in screen output:

```bash
term-cli wait-for --session NAME "error" "success" --ignore-case --capture --timeout 30   # default: 10s
```
...
term-cli request-wait --session NAME --timeout 300            # default: 300s (5 min)
...
If they detach with Ctrl+B d without completing, `request-wait` fails with exit code 4.
...
term-cli wait --session tests --timeout 1800; term-cli capture --session tests --lines 200
...
term-cli wait-idle --session server --seconds 3 --timeout 60 && term-cli capture -s server
# ... later ...
...
term-cli send-text -s debug "p some_var" --enter && term-cli wait -s debug && term-cli capture -s debug
...
- If you lost terminal context, recover via `capture --lines 200` (scrollback is your memory).
...
- Locked sessions (exit code 5): agent can only `capture`, `status`, `wait-*`, and `request`
